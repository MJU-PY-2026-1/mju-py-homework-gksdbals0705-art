...
이 프로그램은 각 사용자에 맞춘 건강 관리 보조가 목표입니다. 이를 위해 우선, 사용자의 이름, 나이, 성별을 입력 받아 정보를 리스트에 저장합니다. 이후 사용자의 몸무게, 키를 입력받아 BMI지수를 계산합니다. BMI지수를 토대로 사용자가 저체중, 정상, 과체중, 비만 중 어디에 속하는지 알려줍니다. 또한, 성별과 나이를 기반으로 BMI 평균값을 도출해 사용자에게 알려줍니다.정상 BMI인 21에 도달하기 위해 얼마만큼의 체중 감량 혹은 증량이 필요한지 계산해줍니다. 매주마다 사용자가 몸무게의 변화를 입력할 경우, 기존 사용자의 몸무게와 비교하여, 사용자에게 올바른 컨설팅을 해주는 것이 목표입니다.
<br>
...
1. 사용자 정보 등록
2. BMI 계산
3. BMI 지수에 따른 저체중, 정상, 과체중, 비만 여부 출력
4. 나이와 성별에 따른 BMI 평균 값
5. 정상 BMI(21)까지 도달 하기 위해 증량, 감량 여부
6. 사용자가 입력한 몸무게 변화와 기존에 계산한 증량(OR감량)에 대해 성공 여부 확인 및 컨설팅
0. 게임 종료
